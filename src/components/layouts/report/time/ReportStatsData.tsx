	...
	return (
		<div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
			<div>
				...
				{summaryStats.timesheetHours > 0 && (
					<p className="text-3xl font-semibold text-gray-900">
						{formatSimpleTime(summaryStats.timesheetHours)}
					</p>
				)}
			...
			<div>
				{/* Time entry Billable/Non-billable (billablePercentage, billableHours, nonBillableHours) commented out
					...
					<div className="relative w-24 h-24">
						<svg className="w-24 h-24 transform -rotate-90">
							<circle
								...
								strokeDashoffset="0"
							/>
							{(summaryStats.timesheetBillablePercentage ?? 0) > 0 && (
								<circle
									cx="48"
									cy="48"
									r="40"
									...
									strokeWidth="8"
									strokeDasharray={`${2 * Math.PI * 40}`}
									strokeDashoffset={`${2 * Math.PI * 40 * (1 - (summaryStats.timesheetBillablePercentage ?? 0) / 100)}`}
								/>
							)}
						</svg>
						<div className="absolute inset-0 flex items-center justify-center">
							<span className="text-sm font-medium text-gray-900">
								{(summaryStats.timesheetBillablePercentage ?? 0).toFixed(0)}%
							</span>
						</div>
					</div>
					<div className="space-y-1">
						<div className="flex items-center space-x-2">
							<div className="w-4 h-4 bg-[#376bdd] rounded"></div>
							<span className="text-sm text-gray-700">
								Billable: {formatDecimalHours(summaryStats.timesheetBillableHours ?? 0).toFixed(2)}
							</span>
						</div>
						<div className="flex items-center space-x-2">
							<div className="w-4 h-4 bg-[#86b1f1] rounded"></div>
							<span className="text-sm text-gray-700">
								Non-billable: {formatDecimalHours(summaryStats.timesheetNonBillableHours ?? 0).toFixed(2)}
							</span>
						</div>
					</div>
				</div>
